...
/* ============================================================
   DOSSIER DE REVISION : pointage persistant
   Une revision se mene sur plusieurs jours, entre deux autres
   taches. Perdre le pointage a chaque fermeture d'onglet rendrait
   la page inutilisable en situation reelle. L'etat vit dans
   localStorage, cle par cle, sans rien envoyer nulle part.
   ============================================================ */
(function () {
  const CLE = 'ohada-revision-v1';

  function initRevision() {
    const cases = Array.from(document.querySelectorAll('[data-revision-check]'));
    if (!cases.length) return;

    const bloc = document.querySelector('[data-revision-progression]');
    const compteur = document.querySelector('[data-revision-fait]');
    const barre = document.querySelector('[data-revision-barre]');
    const reset = document.querySelector('[data-revision-reset]');

    // Le bloc est masque dans le HTML : sans JavaScript une barre de
    // progression figee a zero serait un ornement mensonger.
    ...
    function rafraichir() {
      const faits = cases.filter((c) => c.checked).length;
      if (compteur) compteur.textContent = faits;
      if (barre) barre.style.width = Math.round((faits / cases.length) * 100) + '%';
    }

    const etat = lire();
    cases.forEach((c) => {
      if (etat[c.id]) c.checked = true;
      c.addEventListener('change', () => {
        const courant = lire();
        if (c.checked) courant[c.id] = 1;
        else delete courant[c.id];
        ecrire(courant);
        rafraichir();
      });
    });
